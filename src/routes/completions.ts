...
import { proxyCompletions, getCopilotUrls } from "../proxy/copilotClient.js";
...
export function registerCompletionsRoute(
  ...
  server.post("/v1/completions", async (req, reply) => {
    ...
    if (config.traffic.logCopilotRequest) {
      logEntry({
        ...
        method: "POST",
        url: getCopilotUrls().completions,
        headers: copilotHeaders,
        body: JSON.stringify(body),
      });
    }

    const upstream = await proxyCompletions(body, copilotHeaders);
    ...
    if (isStreaming) {
      await pipeSSEResponse(upstream, reply, config, {
        requestId,
        method: "POST",
        xcodeUrl: "/v1/completions",
        copilotUrl: getCopilotUrls().completions,
